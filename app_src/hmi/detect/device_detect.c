...
void BreakPointSave(uint16_t device_msgId)
{
	...
	switch(device_msgId)
	{
		case MSG_DEVICE_SERVICE_BP_SYS_INFO:
		{
...
#endif
			if((mainAppCt.EffectMode != EFFECT_MODE_HFP_AEC) && (mainAppCt.EffectMode != EFFECT_MODE_USBAEC))//蓝牙通话模式下，不保存音效模式
			{
			...
			pBpSysInfo->MicAec = MicAec;
			pBpSysInfo->Effect3D= Effect3D;
			int i,j;
			for(j=0;j<EQ_MAX_NUM;j++) {
			for(i=0;i<10;i++)	{
				pBpSysInfo->EqGain[j][i]  = EqGain[j][i];
			
			}
			}

			for(i=0;i<10;i++)	{
				pBpSysInfo->earEqGain[i]  = earEqGain[i];
